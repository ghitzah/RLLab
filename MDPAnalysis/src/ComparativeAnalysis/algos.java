...
public class algos {
	...
	private static Comparator<Cluster> distComparator = new Comparator<Cluster>(){
		@Override
		public int compare(Cluster c1, Cluster c2) {
			return (int) (c2.dist_diff - c1.dist_diff);
		}
	};
	
	public static double compute_dist_diff(Cluster c1){
		if (c1.elements.size() < 2) return 0;
...
//				for(Cluster c : new_cls0){
//					if (c.elements.size() > 1) size_queue.add(c);
//				}
//				for(Cluster c : new_cls1){
//					if (c.elements.size() > 1) size_queue.add(c);
//				}
				...
				for(Cluster c : new_cls0){
					if (c.elements.size() > 1) dist_queue.add(c);
				}
				for(Cluster c : new_cls1){
					if (c.elements.size() > 1) dist_queue.add(c);
				}
	...
	private static List<Cluster> declust_T(MDP m, Cluster clarge, Map<Integer,Cluster> membership, Cluster[] oldMembership) {
		...
		for(Integer i : lst) {
			...
			if(!isSet) {
				...
				toRet.add(c);
				c.m = m;
				c.dist_diff = compute_dist_diff(c);
			}
		}
		return toRet;
